...
# Write pattern: "buffered" (default, REALISTIC -- one write() per shard, like
# np.tofile/np.save/torch.save) or "row" (legacy -- one write() per row, i.e.
# N_ROWS syscalls per file). "row" was the original behavior and inflates the POSIX
# op count ~N_ROWS-fold (4096x at ML_ROWS=4096), which is a workload artifact, not
# real ML I/O. Kept only so the old 385k-event baseline stays reproducible.
WRITE_MODE = os.environ.get("ML_WRITE_MODE", "buffered").lower()
...
def write_dataset():
    ...
    for i in range(N_FILES):
        ...
        with open(p, "wb") as f:
            if WRITE_MODE == "row":
                # legacy: one write() per row (N_ROWS syscalls/file -- unrealistic)
                for _ in range(N_ROWS):
                    row = [random.random() for _ in range(N_COLS)]
                    f.write(struct.pack(f"{N_COLS}f", *row))
            else:
                # realistic: build the whole shard, write it in ONE call
                vals = [random.random() for _ in range(N_ROWS * N_COLS)]
                f.write(struct.pack(f"{N_ROWS * N_COLS}f", *vals))
        paths.append(p)
