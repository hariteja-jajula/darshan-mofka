...
for v in WORKLOAD EVENTS CHECKPOINTS REPS DARSHAN_MOFKA_ENABLE DARSHAN_MOFKA_TIMING \
         ...
         MOFKA_PROTOCOL RPC_THREAD_COUNT DRAIN_WAIT_S CONSUMERS NA_OFI_TX_SIZE NA_OFI_RX_SIZE MOFKA_NA_DOMAIN MOFKA_CLIENT_MODE DARSHAN_MOFKA_FINAL_SWEEP \
         DARSHAN_MOFKA_ASYNC DARSHAN_MOFKA_QUEUE_DEPTH DARSHAN_MOFKA_DROP_POLICY DARSHAN_MOFKA_DRAIN_THREADS DARSHAN_MOFKA_BATCH DARSHAN_MOFKA_MAX_BATCHES DARSHAN_MOFKA_JOIN_MS DARSHAN_MOFKA_FLUSH_MS DARSHAN_MOFKA_VERBOSE; do
    [ -n "${!v:-}" ] && FWD="${FWD:+$FWD,}$v=${!v}"
done
