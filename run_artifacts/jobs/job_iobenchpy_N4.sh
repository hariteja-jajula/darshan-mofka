...
# Results -> results/OVH_iobenchpy_N4/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_iobenchpy_N4; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug-scaling \
     -l select=5:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_iobenchpy_N4 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=io_bench_py; PROTO=ofi+cxi; NODES=5; TASKS=1
...
run_arm() {  # $1=arm $2=reps $3=enable $4=nodarshan $5=sender
  ...
  MOFKA_PROTOCOL="$PROTO" WORKLOAD="$WL" NODES="$NODES" TASKS="$TASKS" REPS="$2" PLACEMENT=separate \
    ...
    RESULTS_TAG="OVH_iobenchpy_N4/$1" \
