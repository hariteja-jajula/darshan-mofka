#!/bin/bash
# job_iobenchpy_N1.sh -- overhead study: io_bench_py (Python, CXI), 1 workload node.
...
# Results -> results/OVH_iobenchpy_N1/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_iobenchpy_N1; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug \
     -l select=2:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_iobenchpy_N1 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=io_bench_py; PROTO=ofi+cxi; NODES=2; TASKS=1
CMP=10; MAT=256; EV=100; PART=4; CONS=1; RPC=0
...
run_arm() {  # $1=arm $2=reps $3=enable $4=nodarshan $5=sender
  ...
  MOFKA_PROTOCOL="$PROTO" WORKLOAD="$WL" NODES="$NODES" TASKS="$TASKS" REPS="$2" PLACEMENT=separate \
    ...
    RESULTS_TAG="OVH_iobenchpy_N1/$1" \
