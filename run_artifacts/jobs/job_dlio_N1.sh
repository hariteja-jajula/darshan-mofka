...
# Results -> results/OVH_dlio_N1/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_dlio_N1; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug \
     -l select=2:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_dlio_N1 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=dlio; PROTO=ofi+tcp; NODES=2; TASKS=32
