...
# Results -> results/OVH_mpi_N4/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_mpi_N4; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug-scaling \
     -l select=5:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_mpi_N4 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=mpi; PROTO=ofi+tcp; NODES=5; TASKS=32
