#!/bin/bash
# job_iobench_N1.sh -- overhead study: io_bench (C, CXI), 1 workload node.
...
# Results -> results/OVH_iobench_N1/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_iobench_N1; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug \
     -l select=2:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_iobench_N1 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=io_bench; PROTO=ofi+cxi; NODES=2; TASKS=1
CMP=58; MAT=512; EV=100; PART=4; CONS=1; RPC=0
