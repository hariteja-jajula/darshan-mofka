#!/bin/bash
# job_iobenchpy_N1.sh -- overhead study: mpi (MPI-IO, TCP), 1 workload node (32 ranks).
...
# Results -> results/OVH_mpi_N1/<arm>/RUN*.  ~7 min/rep (fits 1h debug).
set -euo pipefail
...
STUDY=OVH_mpi_N1; mkdir -p "$ROOT/results/$STUDY"

qsub -A "$account" -q debug \
     -l select=2:ncpus=32:mpiprocs=32 -l walltime=01:00:00 \
     "${PBS_EXTRA[@]}" -N 3arm_mpi_N1 -j oe -o "$ROOT/results/$STUDY/" <<'PBS'
...
WL=mpi; PROTO=ofi+tcp; NODES=2; TASKS=32
CMP=0; MAT=256; EV=560; PART=16; CONS=16; RPC=4
