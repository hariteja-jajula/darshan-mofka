...
# Respect a ROOT set by the caller (e.g. workloads/HEP/run_HEP.sh, which lives two
# levels deep); otherwise derive it one level up from the sourcing script
# (overhead_study/*.sh live one level deep).
if [ -z "${ROOT:-}" ]; then
