...
Run the streaming steps on a **compute node** (the Mofka broker's fabric
...
Refresh the environment after building Diaspora. This is required because the
install step creates `diaspora-stream-api/install/lib/python3.14/site-packages`,
which must be on `PYTHONPATH` for Mofka's Python client to import
...
## 3. Build Darshan and the demo workloads
...
That group file is what the Darshan connector and the consumer both use to
connect to the same Mofka server.

## 5. Choose output files

Pick where this run writes its FlowCept artifacts and exported JSONL. Use
...
Start FlowCept before the workload. It runs in the background and continuously
...
> login node* and (2) live on a **shared filesystem the compute nodes can see
> (`eagle`)** -- not under `$HOME`. A `mongod` in a `$HOME` conda env will fail
> the `[[ -x ]]` check on a compute node. Put it under the repo (which is on
> `eagle`) and point `MONGOD` at it.
...
If one is found and runs (`"$MONGOD" --version` prints and
`ldd "$MONGOD" | grep -i "not found"` is empty), just point at it:

```bash
export MONGOD=/eagle/<proj>/<user>/.../envs/<mongo-env>/bin/mongod
```

Otherwise get one -- **on a login node** (internet + `$HOME` both available
there), installing INTO the repo on `eagle`:
...
Run any workload under Darshan while FlowCept is draining the topic. This example
uses the C smoke workload:
...
Expected: the workload prints `mofka_forward_smoke complete...` and the send count
is nonzero.
...
Tell FlowCept to flush and stop its consumer, then export the workload-specific
MongoDB records to JSONL:
...
If you exported a DLIO run to a separate JSONL file, point `EVENTS_JSONL` at that
file and verify it the same way. See [workloads/dlio/README.md](../workloads/dlio/README.md).
...
If the normal Darshan shutdown path fails and no final `.darshan` log is
produced, the captured stream can be converted into a best-effort partial Darshan
log:
...
The reconstructed log is intentionally marked partial. It contains the latest
module record snapshots that reached Mofka, plus synthetic job/exe/mount metadata.
